...
console.log("[Workday Autofill] Content script loaded on:", window.location.href);

chrome.storage.sync.get(["enabled_global", "enabled_domains", "confirmed_domains"], (data) => {
  const domain = window.location.hostname.split(".")[0]; // e.g. "github" from "github.myworkdayjobs.com"
  const domainAllowed = (data.enabled_domains || {})[domain] !== false;

  if (data.enabled_global === false) return;       // globally disabled

  if (data.confirmed_domains?.[domain]) {          // already consented to this company
    startAutofill();
